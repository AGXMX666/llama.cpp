	...
	>
		...
		<span class="inline-flex flex-col items-start gap-1 text-left">
			...
			<span class="text-xs text-muted-foreground">
				({t('{count} resource{plural}', {
					count: serverRes.resources.length,
					plural: serverRes.resources.length !== 1 ? 's' : ''
				})}{#if hasTemplates}, {t('{count} template{plural}', {
					count: serverRes.templates.length,
					plural: serverRes.templates.length !== 1 ? 's' : ''
				})}{/if})
			</span>
		</span>
		...
		<div class="ml-4 flex flex-col gap-0.5 border-l border-border/50 pl-2">
			{#if serverRes.error}
				<div class="py-1 text-xs text-red-500">
					{t('Error: {message}', { message: serverRes.error })}
				</div>
			{:else if !hasContent}
