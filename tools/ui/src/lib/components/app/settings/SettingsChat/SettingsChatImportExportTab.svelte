			...
			input.onchange = async (e) => {
				...
				try {
					...
					if (importedData.length === 0) {
						throw new Error(t('No conversations found in file'));
					}
	...
	async function handleImportConfirm(selectedConversations: DatabaseConversation[]) {
		try {
			...
			if (skipped.length > 0) {
				toast.info(
					skipped.length === 1
						? t('Skipped 1 conversation already in your library')
...
<DialogConfirmation
	...
	description={t('Are you sure you want to delete all conversations? This action cannot be undone and will permanently remove all your conversations and messages.')}
