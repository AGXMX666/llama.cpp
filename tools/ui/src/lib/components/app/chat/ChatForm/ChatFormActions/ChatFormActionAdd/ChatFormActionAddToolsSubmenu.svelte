			...
			{:else if toolsStore.isToolsEndpointUnreachable}
				<div class="grid gap-2.5 px-3 py-4 text-sm text-muted-foreground">
					<span class="flex gap-2">
						...
						<span>
							{t('Run llama-server with')} <code>{CLI_FLAGS.TOOLS}</code> {t('flag to enable')}
					...
					<span class="flex gap-2">
						...
						<span>
							{hasMcpServersAvailable ? t('Enable') : t('Add')} {t('MCP Server(s) to access')}
								...
								<Tooltip.Content side="right">
									<p>
										...
										{group.tools.length === 1
											? t('{count} tool', { count: group.tools.length })
