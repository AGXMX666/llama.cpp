	...
	{#if isSwitchable}
		<div class="inline-flex items-center rounded-sm bg-muted-foreground/15 p-0.5">
			...
			{@render viewButton({
				...
				icon: Sparkles,
				label: t('Generation'),
				tooltipText: t(isGenerationDisabled ? 'Waiting for tokens...' : 'Generation'),
	...
	<div class="flex items-center gap-1 px-2">
		{#if activeView === ChatMessageStatsView.GENERATION && hasGenerationStats}
			<ChatMessageStatisticsBadge
				...
				icon={WholeWord}
				value={t('{count} tokens', { count: predictedTokens?.toLocaleString() ?? 0 })}
			...
			<ChatMessageStatisticsBadge
				class="bg-transparent"
				icon={Gauge}
				value={t('{rate} t/s', { rate: tokensPerSecond.toFixed(2) })}
		...
		{:else if activeView === ChatMessageStatsView.TOOLS && hasAgenticStats}
			<ChatMessageStatisticsBadge
				class="bg-transparent"
				icon={Wrench}
				value={t('{count} calls', { count: agenticTimings!.toolCallsCount })}
			...
			<ChatMessageStatisticsBadge
				class="bg-transparent"
				icon={Gauge}
				value={t('{rate} calls/s', { rate: agenticToolsPerSecond.toFixed(2) })}
		...
		{:else if activeView === ChatMessageStatsView.SUMMARY && hasAgenticStats}
			<ChatMessageStatisticsBadge
				class="bg-transparent"
				icon={Layers}
				value={t('{count} turns', { count: agenticTimings!.turns })}
		...
		{:else if hasPromptStats && (mode === ChatMessageStatisticsMode.READING || isSwitchable)}
			<ChatMessageStatisticsBadge
				class="bg-transparent"
				icon={WholeWord}
				value={t('{count} tokens', { count: promptTokens ?? 0 })}
			...
			<ChatMessageStatisticsBadge
				class="bg-transparent"
				icon={Gauge}
				value={t('{rate} tokens/s', { rate: promptTokensPerSecond!.toFixed(2) })}
