...
async function parseErrorMessage(response: Response): Promise<string> {
	...
	if (httpErrorStr) {
		return t(httpErrorStr);
	}

	return t(`${ERROR_MESSAGES.HTTP.GENERIC}: {status} {statusText}`, {
...
function beautifyNetworkError(throwable: unknown): string {
	...
	if (throwable instanceof Error) {
		...
		if (throwable.name === 'TypeError' && message.includes('fetch')) {
			return t(ERROR_MESSAGES.NETWORK.UNREACHABLE);
		}
	} else {
	...
	if (message.includes('ECONNREFUSED')) {
		return t(ERROR_MESSAGES.NETWORK.REFUSED);
	} else if (message.includes('ENOTFOUND')) {
		return t(ERROR_MESSAGES.NETWORK.NXDOMAIN);
	} else if (message.includes('ETIMEDOUT')) {
		return t(ERROR_MESSAGES.NETWORK.TIMEOUT);
	}

	return t('{generic} ({message})', {
		generic: t(ERROR_MESSAGES.NETWORK.GENERIC),
		message
	});
}
